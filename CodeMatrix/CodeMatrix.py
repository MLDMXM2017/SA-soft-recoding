...
from CodeMatrix.Criterion import *
    ...
    while len(labels_to_agg_list) > 1:
        ...
        for i in range(0, len(labels_to_agg_list) - 1):
            for j in range(i + 1, len(labels_to_agg_list)):
                ...
                score = agg_score(class_1_data, class_1_label, class_2_data, class_2_label,
                                            score=max_distance_score)
...
def cl_ecoc(X, y):
    ...
    while len(labels_to_divide) > 0:
        label_set = labels_to_divide.pop(0)
        datas, labels = _get_data_subset(X, y, label_set)
        class_1_variety_result, class_2_variety_result = sffs(datas, labels, score=max_center_distance_score)

...
class OrdinaryDecoder(Decoder):
    ...
    def decode(self, Y, M, us=None, ts=None):
        Y, M = self._check_param(Y, M)
        if us is None:
            return np.array([[self._distance(y, M[i, :]) for i in range(M.shape[0])] for y in Y])
        else:
            return np.array([[self._distance(y, M[i, :], self._calculate_p(y, us[i, :], ts[i, :])) for i in range(M.shape[0])] for y in Y])
    @staticmethod
    def _calculate_p(y, us, ts):
        b = np.array([stats.norm(us[i], ts[i]).pdf(y[i]) for i in range(len(y))])
        a = 1 - b
        # print(b)
        # print(a)
        return np.abs(a)
...
class WeightedDecoder(Decoder):
    ...
    def decode(self, Y, M, W, us=None, ts=None):
    ...
    @staticmethod    
    def _calculate_p(y, us, ts):
        return np.array([stats.norm(us[i], ts[i]).pdf(y[i]) for i in range(len(y))])
...
class ExponentialLossBasedDecoder(OrdinaryDecoder):
    ...
    def _distance(y, m, p=None):
        if p is None:
            ...
            return sum(p * np.exp(-1 * np.sign(y * m) * (np.abs(m))))
